...
( function(){
    ...
    function createXHRRequest(url, params){
        ...
        xhr.open('GET', url + (findGetParameter('disableCache') ? "?t=" + Date.now() : ''), true);
        xhr.send();
        return xhr;
    }
        ...
        src: function(src){
            ...
            for (var i in src){
                 for (var j in src[i]){
                    ...
                    var script = document.createElement("script"), ssrc = link + src[i][j] + (findGetParameter('disableCache') ? "?t=" + Date.now() : '');
                    script.src = ssrc;
                    script.async = false;
